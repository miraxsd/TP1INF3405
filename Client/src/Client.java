...
public class Client {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			// 1. Crï¿½ation d'une nouvelle connexion avec le serveur
			// String serverAddress="127.0.0.1";
			// int port=5000;

			try {
				socket = new Socket(serverAddress, port);
				// Attendre que le client soit connectï¿½ au serveur
				// Si la connection est timed out redemander au client d'entrer une nouvelle
				// adresse du serveur
				synchronized (socket) {
					socket.notify();
				}
			} catch (java.net.ConnectException e) {
				System.out.println("Impossible de se connecter ï¿½ ce serveur");
				serverAddress = "";
				port = 0;
				continue;
			}
		...
		// Crï¿½ation d'un canal entrant pour recevoir les messages envoyï¿½s par le
		// serveur
		...
		// Crï¿½ation d'un canal sortant pour ï¿½crire au serveur
		DataOutputStream out = new DataOutputStream(socket.getOutputStream());
		PrintStream ps = new PrintStream(socket.getOutputStream());
		String strKb = "";

		// Saisir entrï¿½e du client au clavier
		...
		while ((strKb = kb.nextLine()) != null) {
			...
			case "download":
				...
				if (in.readUTF().equals("ready")) {
					...
					System.out.println(
							"Aucun fichier nommï¿½ " + command[1] + " n'existe dans le dossier actuel du serveur");
				in.readNBytes(in.available());// Vider le InputStream
				break;
			default:
			...
			strKb = "";
			command[0] = "";
			if (command.length > 1) // Si la commande a un deuxieme argument on le rï¿½initialise
				command[1] = "";
		}
	...
	public static int readPort(Scanner sc) {
		int port = 0;
		System.out.println("Entrez numï¿½ro de port du serveur :");
		if (sc.hasNextInt()) {
			port = sc.nextInt();
		} else {
			sc.next();
		...
		while (port > 5050 || port < 5000) {
			System.out.print("Numï¿½ro de port invalide. Veuillez enter un numï¿½ro de port entre 5000 et 5050 :");
			if (sc.hasNextInt()) {
				port = sc.nextInt();
			} else {
